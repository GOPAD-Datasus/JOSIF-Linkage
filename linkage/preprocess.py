...
def preprocess() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        sinasc = pd.read_parquet(input_fdr + "DN.parquet.gzip")
    except FileNotFoundError:
        error = "Missing DN.parquet.gzip inside input folder"
        raise RuntimeError(error)

    try:
        sim = pd.read_parquet(input_fdr + "DO.parquet.gzip")
    except FileNotFoundError:
        error = "Missing DO.parquet.gzip inside input folder"
        raise RuntimeError(error)
