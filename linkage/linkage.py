...
def link(sinasc: pd.DataFrame, sim: pd.DataFrame):
    ...
    df_mort = df_mort.merge(sinasc, left_on="index_1", right_on="index", how="left")
    df_mort = df_mort.merge(sim, left_on="index_2", right_on="index", how="left")

    output_fdr = "data/output/"
    df_mort.to_parquet(output_fdr + "SIMORTN.parquet.gzip", compression="gzip")
